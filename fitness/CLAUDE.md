...
- Le bloc des pages de famille sur le hub **ne prétend jamais être l'inventaire de l'offre**. Il ne s'intitule pas "Nos disciplines" : ces quatre pages existent parce que ce sont les disciplines à plusieurs variantes, pas parce que ce sont les seules. Le titre le dit, et une phrase renvoie au catalogue pour le reste (Q40).
...
- **Un club propose une forme ou l'autre, jamais les deux** : Cross Training est soit un cours collectif inclus, soit un Small Group Training payant, selon le club (client, 2026-09-16). Aujourd'hui Versoix en cours collectif, La Praille et Pâquis en Small Group Training, les autres clubs n'en proposent pas. C'est une règle de cohérence à vérifier dans les données, pas seulement une situation de fait.
