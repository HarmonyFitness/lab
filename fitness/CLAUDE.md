...
- **La règle vaut pour toute ligne d'inclusion, pas seulement pour l'accès aux clubs**, et elle ne se saisit pas : `HF.vues.ligneInclusion(id)` la déduit des formules. Trois réponses possibles, "dans toutes les formules", "dès la formule X", ou la ligne des Extras quand aucune formule ne la comprend. Une phrase écrite dans une page est une phrase que personne ne corrigera.
- **Ce qui se vend en Extra dit aussi par où on y échappe** : "En Extra de votre abonnement ou inclus dans la formule Premium Platinum" (`HF.vues.ligneExtra()`, Hugo, 2026-09-16). Dans cet ordre : ce que ça coûte d'abord, la sortie par le haut ensuite. Le nom de la formule est déduit de `inclutExtras`, jamais écrit dans une page.
...
- **Une ligne d'inclusion « Espaces wellness »** (Hugo, 2026-09-17). C'est un écart réel entre les formules et il n'apparaissait nulle part : Gym n'en a aucun, Essential ouvre le sauna et le hammam, Premium ouvre les cinq. La ligne est **déduite**, jamais saisie sur un produit : ce sont les espaces des clubs que la catégorie couvre. Elle utilise la même grammaire de tags que la ligne Extras, quatre au plus puis on compte le reste, pour qu'une carte reste comparable à sa voisine.
...
- **Une carte sans club porte quand même une action** : « Choisir mon club », qui descend au bloc du club (Hugo, 2026-09-16). Sans elle, la carte n'avait aucun CTA et la page ne disait pas quoi faire. Ce n'est pas un bouton de souscription, la règle de B.3 tient : on ne souscrit pas sans club référent. La ligne d'explication a disparu du même coup, elle aurait dit deux fois la même chose.
...
- Le club se choisit avec le composant de Tarifs : la question est « lequel est près de chez moi », pas « lequel me concerne ». C'est ce qui distingue cette page de `/nous-contacter`, qui route un message avec un simple menu. « Même gabarit » dans B.3 veut dire même structure, pas mêmes champs.
- Le club fait partie de la demande, **sous le même titre que le formulaire**, pas en section à part : le sortir repoussait le formulaire d'un écran alors que c'est lui l'action de la page. Il garde sa pleine largeur (les champs sont étroits, une carte ne l'est pas) et le composant sait se passer de son propre titre (`sansTitre`), sinon deux titres se suivraient pour un seul bloc.
- Hero : une photo et un bouton d'ancre vers la demande. On demande à quelqu'un de pousser la porte d'un club qu'il ne connaît pas, et une page sans visage ne donne envie à personne (Hugo, 2026-09-16). La photo montre l'accueil, pas l'effort : le moment du hero est celui où on entre, pas celui où on souffre.
- B.3 ne donne pas la copy de cette page. Le H1 pose la question que se pose vraiment quelqu'un qui n'est jamais venu, « Venez voir si c'est fait pour vous », et le sous-titre répond avec le déroulé réel, pas avec une promesse en l'air. Marqué à valider par Harmony, comme tout texte rédigé et pas encore validé.
...
- Là où l'Extra s'affiche (page Tarifs, fiche club), il **nomme les Small Group Training**, lus sur le planning, à la place d'une description générique. Le visiteur voit ce qu'il achète, et ça reste un seul produit.
- **Sur la page Tarifs, deux lignes, pas une** (Hugo, 2026-09-22). « Dans votre club référent » et « Dans tous les clubs de la formule [nom] » répondent à deux questions différentes, et une seule liste répondait à la première en laissant croire qu'elle répondait aux deux. C'est l'argument de l'Extra sur une formule haute : depuis Meyrin, Premium ouvre sept trainings là où le club référent en propose deux.
- La portée de la deuxième ligne vient de `categorie.couvre` de **la formule choisie**, jamais de la catégorie du club référent : depuis un club Essential on peut souscrire Premium, et l'Extra ouvre alors les dix clubs. Tant qu'aucune formule n'est choisie, on ne peut pas la nommer, donc on ne la nomme pas : la ligne montre le catalogue entier et dit que la formule décide.
- **Deux lignes identiques ne sont pas deux informations.** Quand le club référent propose déjà tout ce que la formule ouvre, une seule ligne s'affiche. C'est le cas de Pâquis en formule Gym, qui ne couvre que Pâquis.
- Sur une **page club**, la question « et ailleurs ? » ne se pose pas : la page parle d'un club, l'Extra garde une ligne, « Dans ce club » (`carteExtra(e, etat, { portee: 'club' })`).
- Sans club référent, une ligne, « Selon les clubs », avec tous les trainings du catalogue.
- Le libellé de portée est en gras et en noir, la liste en gris : quand deux lignes se suivent, c'est la portée qu'on cherche des yeux, pas le premier nom de la liste.
